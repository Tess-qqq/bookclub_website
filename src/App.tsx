...
function BookCover({ title, author, size = 'md' }: { title: string; author: string; size?: 'sm' | 'md' | 'lg' }) {
  ...
  const sizes = { sm: 'w-10 h-14', md: 'w-12 h-16', lg: 'w-16 h-22' };
  ...
  return (
      <div className="space-y-10 pb-8">
        ...
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.28 }}
                    className="pt-6 border-t border-white/6 space-y-5">
          {[
            { t: 'One book, everyone reads', d: 'Each event we agree on a single book. No pressure, no deadlines — just read at your own pace.' },
            { t: 'You pick what\'s next',     d: 'Nominate books, then vote. The most-voted book becomes the next pick.' },
            { t: 'Three campuses, one club',  d: 'AMU, AITU, and NU each have their own list but we share the same spirit.' },
          ].map(({ t, d }) => (
              <div key={t} className="flex gap-4">
                <div className="w-1 rounded-full bg-amber-500/30 flex-shrink-0 mt-1" />
                <div>
                  <p className="font-semibold text-white/80 text-sm">{t}</p>
                  <p className="text-white/35 text-sm leading-relaxed mt-0.5">{d}</p>
                </div>
              </div>
          ))}
        </motion.div>
